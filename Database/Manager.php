...
class Manager
{
    ...
    {
        $this->connection = \York\Dependency\Manager::getDatabaseConnection();
        $this->connection
            ->setHost(\York\Dependency\Manager::getDatabaseConfiguration()->get('db_host'))
            ->setUser(\York\Dependency\Manager::getDatabaseConfiguration()->get('db_user'))
            ->setPassword(\York\Dependency\Manager::getDatabaseConfiguration()->get('db_pass'))
            ->setSchema(\York\Dependency\Manager::getDatabaseConfiguration()->get('db_schema'))
            ->connect();
    }
    ...
    public function query(\York\Database\QueryBuilder\QueryStringInterface $queryStringObject)
    {
        ...
        try {
            ...
            \York\Dependency\Manager::getLogger()->log('query exception: ' . $exception->getMessage(), \York\Logger\Level::DATABASE_ERROR);
        }
     ...
     *
     * @param \York\Database\QueryResult    $queryResultObject
     * @param float                         $start
     * @param float                         $end
     *
     * @todo refactor...
     */
    protected function log(\York\Database\QueryResult $queryResultObject, $start, $end)
    {
        ...
        if (false === $queryResultObject->queryWasSuccessful()) {
            \York\Dependency\Manager::getLogger()->log($logtext, \York\Logger\Level::DATABASE_ERROR);
        }
        if ('1' !== \York\Dependency\Manager::getApplicationConfiguration()->getSafely('disable_db_log') || true === $this->forceLogging || (int)$execution > 1) {
            \York\Dependency\Manager::getLogger()->log($logtext, \York\Logger\Level::DATABASE_DEBUG);
        }
    }
