...
abstract class CoreModel{
	 ...
	 */
	public static function Factory($type = 'Model', DatabaseManager $manager = null){
		$type = ucfirst($type);
		if(false === AutoLoader::isLoadable($type)){
			throw new ApocalypseException(sprintf('Model "%s" not found!', $type));
		}
		$instance = new $type($manager);

		if(false === $instance instanceof CoreModel){
			throw new ApocalypseException(sprintf('Model "%s" not an instance of the core model!!', $type));
		}
	 ...
	 * @return integer
	 */
	protected function sortBy($key, $one, $two){
