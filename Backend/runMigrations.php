...
use York\Console\Flag;
use York\Helper\Application as Helper;
 ...
 */
class runMigrations extends Application{
	/**
	...
	protected function grabAction(){
		...
		if(true === Flag::Factory('finished', 'f')->isEnabled()){
			$this->listFinished();
			return;
		}

		if(true === Flag::Factory('list', 'l')->isEnabled()){
			$this->listAll();
			return;
		}

		if(true === Flag::Factory('new', 'n')->isEnabled()){
			$this->listNew();
			return;
		}


		if(true === Flag::Factory('simulate', 's')->isEnabled()){
			$this->simulate();
			return;
		}

		if(true === Flag::Factory('execute', 'e')->isEnabled()){
			$this->execute();
			return;
		}

		if(true === Flag::Factory('create', 'c')->isEnabled()){
	...
	protected function createMigration(){
		...
		$this->verboseOutput('migration '.$migrationNumber.' created');
	}
	...
	protected function getPathToMigrations(){
		return Helper::getApplicationRoot().'migrations/';
	}
