...
abstract class Manager implements ManagerInterface{
	...
	public static function login($result = null){
		...
		if(null === $result){
			...
			\York\Dependency\Manager::get('splashManager')->addText(\York\Helper\Translator::translate('Unbekannter Nutzer'));
		...
		if($result->password !== md5($dataObject->get(self::get(\York\Configuration::$AUTH_CREDENTIAL_USER_ACCESS)))){
			self::checkBanAfterFailedLogin($result);
			\York\Dependency\Manager::get('splashManager')->addText(\York\Helper\Translator::translate('Falsches Passwort!'));
			\York\Helper\Application::redirect('/auth/login');
		}
		if(\York\Configuration::$USER_STATUS_BANNED == $result->status){
			\York\Helper\Application::redirect('/error/banned');
		}
		if(\York\Configuration::$USER_STATUS_PENDING == $result->status){
			\York\Helper\Application::redirect('/auth/pending');
		}
